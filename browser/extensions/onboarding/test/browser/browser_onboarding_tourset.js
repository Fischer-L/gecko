...
requestLongerTimeout(99); // TMP

add_task(async function test_onboarding_default_new_tourset() {
  resetOnboardingDefaultState();

  let tab = await openTab("https://example.com"); // ABOUT_NEWTAB_URL
  ok(content, "content is there");
  ok(content.window.location.href, "content.window.location: " + content.window.location.href);
  ...
  ok(window, window.document.documentElement.getAttribute("windowtype"));
